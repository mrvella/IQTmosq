...
for(i in year){
  assign(paste0("SS.", "1534.", i), subset(hasGis, format.Date(newDate, "%Y")==i & (hasGis$F1534C_converted=="SS")))
  assign(paste0("SR.", "1534.", i), subset(hasGis, format.Date(newDate, "%Y")==i & (hasGis$F1534C_converted=="SR")))
  assign(paste0("RR.", "1534.", i), subset(hasGis, format.Date(newDate, "%Y")==i & (hasGis$F1534C_converted=="RR")))
}

# Loop to subset on year and genotype for locus 1016
for(i in year){
  #dots <- vector()
  assign(paste0("SS.", "1016.", i), subset(hasGis, format.Date(newDate, "%Y")==i & (hasGis$V1016I_converted=="SS")))
  assign(paste0("SR.", "1016.", i), subset(hasGis, format.Date(newDate, "%Y")==i & (hasGis$V1016I_converted=="SR")))
  assign(paste0("RR.", "1016.", i), subset(hasGis, format.Date(newDate, "%Y")==i & (hasGis$V1016I_converted=="RR")))
...
class(myplots)
l = mget(myplots)
invisible(mapply(ggsave, file=paste0("maps/map_", Sys.Date(), "_", names(l), ".png"), plot=l))

# Save image of the 1016 maps
ls()
objects = ls()
myplots <- objects[135:149]
class(myplots)
l = mget(myplots)
invisible(mapply(ggsave, file=paste0("maps/map_", Sys.Date(), "_", names(l), ".png"), plot=l))
