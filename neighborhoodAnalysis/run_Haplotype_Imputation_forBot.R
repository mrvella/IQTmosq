...
freq2000 <- sum2000/bot$n[1]
freq2001 <- sum2001/bot$n[2]
freq2002 <- sum2002/bot$n[3]
freq2003 <- sum2003/bot$n[4]
freq2004 <- sum2004/bot$n[5]
freq2005 <- sum2005/bot$n[6]
freq2006 <- sum2006/bot$n[7]
freq2007 <- sum2007/bot$n[8]
freq2008 <- sum2008/bot$n[9]
freq2009 <- sum2009/bot$n[10]
freq2010 <- sum2010/bot$n[11]
freq2011 <- sum2011/bot$n[12]
freq2012 <- sum2012/bot$n[13]
freq2013 <- sum2013/bot$n[14]
freq2014 <- sum2014/bot$n[15]
freq2015 <- sum2015/bot$n[16]
freq2016 <- sum2016/bot$n[17]
freq2017 <- sum2017/bot$n[18]
...
# Save df to .csv file
write.csv(freqAll, "freqAll_bot.csv", row.names = FALSE)

### Calculate 95% Confidence Interval (not for ribbon CIs) ------------------------------------------------------------
## for column 1: SS
df <- data.frame()
for(i in 1:18){
  ci = 1.96 * (sqrt((freqAll[i,1]*(1-freqAll[i,1]))/(2*bot$n[i])))
  df = rbind(df, ci)
}
colnames(df) <- "CI_95"
CI_95.SS <- df
...
df <- data.frame()
for(i in 1:18){
  ci = 1.96 * (sqrt((freqAll[i,2]*(1-freqAll[i,2]))/(2*bot$n[i])))
  df = rbind(df, ci)
}
colnames(df) <- "CI_95"
CI_95.SR <- df
...
df <- data.frame()
for(i in 1:18){
  ci = 1.96 * (sqrt((freqAll[i,3]*(1-freqAll[i,3]))/(2*bot$n[i])))
  df = rbind(df, ci)
}
colnames(df) <- "CI_95"
CI_95.RS <- df
...
df <- data.frame()
for(i in 1:18){
  ci = 1.96 * (sqrt((freqAll[i,4]*(1-freqAll[i,4]))/(2*bot$n[i])))
  df = rbind(df, ci)
}
colnames(df) <- "CI_95"
CI_95.RR <- df
...
write.csv(freqAll_long, "freqAll_long_bot.csv", row.names = FALSE)
...
# Write plot to png
ggsave(filename = paste0("/Users/jenbaltz/Dropbox/GouldLab/Project_Mosquito/Database/figures/kdrHaps/kdrNeighborhoods_bars/bot_bars_", Sys.Date(), ".png"), width = 11, height = 8, dpi = 600, units = "in", device='png')
