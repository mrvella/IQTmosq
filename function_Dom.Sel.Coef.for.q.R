...
# Solves for s in Table 2.2, equation 4 - column heading "Change of gene frequency" 
...
# To Calculate s
Dom.Sel.Coef.for.q = function (q.1, q.2, g) {
  delta.q = (q.2-q.1)/g
  s = (delta.q)/((q.1^2)-(q.1^3)+(delta.q * (1-q.1^2)))
}


# To plot s
plot.Dom.Sel.Coef.for.q = function(q.1, q.2, g, s, dataframe) {
  delta.q = (q.2-q.1)/g
  df.name <- deparse(substitute(dataframe))
  png(file = print(paste0(df.name,"Dom-forq.png")), units = "px", height = 600, width = 900)
  scatterplot3d(delta.q, q.1, abs(s),highlight.3d = TRUE, col.axis = "blue", 
                ...
                main = "Dominance of p, Selection for q", xlab = "Delta q", ylab = "", 
